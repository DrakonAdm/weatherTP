from terminator.weatherTerminator.models import Forecast
...
def forecastClothes(queryset):
    ...
    if queryset[0].windSpeed > 20:
        return "Слишком сильный ветер", "Слишком сильный ветер", "Сиди дома"

    if queryset[0].precipitation > 30:
        return "Слишком много осадков", "Сиди дома", "Или не забудь зонт, но он не поможет)"
    ...
    temperature['hotTwo'] = fuzz.trimf(temperature.universe, [24, 26, 28])
    temperature['hotOne'] = fuzz.trimf(temperature.universe, [28, 29, 30])
    ...
    return maxStr, minStr, averageStr
