...
import {getAd, getClothes, getWeatherTomorrow} from "../http/weatherApi";
...
const Tomorrow = observer(() => {
    ...
    useEffect(() => {
        getWeatherTomorrow("tomorrow", user.chooseCity).then(day => data.setWeatherDay(day))
                ...
                <Row>
                    <Col md={9}>
                        <Card style={{width: 800, borderWidth: 0, backgroundColor: "transparent"}} className="p-4 d-flex">
                            <h2>Прогноз погоды на завтра, {user.chooseCity}</h2>
