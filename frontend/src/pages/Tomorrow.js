...
const Tomorrow = observer(() => {
    ...
    return (
        <Container>
            ...
            <Card style={{width: 1000, borderWidth: 0, backgroundColor: "#FFFAF4"}} className="p-4 d-flex">
                <h3>{data.weather[2].date}</h3> {/*здесь как-то надо получать определенный день, чтобы для него уже выводить все*/}
                <CardGroup style={{width: 900}}>
                    <Card style={{borderWidth: 3, backgroundColor: "#EFF8FF"}} className="p-4 text-center">
                        ...
                        <h3>Днем: {data.weather[2].maxTemp}</h3>
                        <h4>Ночью: {data.weather[2].minTemp}</h4>
                    </Card>
                    <Card style={{borderWidth: 3, backgroundColor: "#EFF8FF"}} className="p-4 text-center">
                        <h7>Давление: {data.weather[2].pressure}</h7>
                        <h7>Скорость ветра: {data.weather[2].windSpeed} </h7>
                        <h7>Осадки: {data.weather[2].rainfall}</h7>
                    </Card>
                </CardGroup>
